...
#define WFPadding               (10.0f)
#define WFColumnSpace           (35.0f) // The space between columns
#define WFColumnWidth           ((self.frame.size.width - (self.columns - 1) * WFColumnSpace - 2 * WFPadding) / self.columns)

#define WFCellSpace             (20.0f) // The space between cells
...
- (void)initialize
{
    ...
    for (int i = 0; i < self.columns; ++i) {
        ...
        CGFloat heightTillNow = WFPadding;
        CGFloat originX = self.frame.origin.x + i * (WFColumnWidth + WFColumnSpace) + WFPadding;
...
#pragma mark - WaterFlowViewCell delegate

- (void)didSelectedCell:(WaterFlowViewCell *)cell {
    [self.waterFlowDelegate waterFlowView:self didSelectRowAtIndexPath:cell.indexPath];
}
